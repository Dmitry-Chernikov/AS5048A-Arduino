 ...
 */
word AS5048A::getRawRotation(byte sizeValues, bool median = false, bool circular = false){
  return read(static_cast<word>(AS5048A_ANGLE), sizeValues, median, circular);
}
...
void AS5048A::ProgAbsolAngleZeroPosit(){
  ...
  if ( (-1 < AS5048A::getRawRotation()) && (AS5048A::getRawRotation() < 1) ){
 ...
 */
void AS5048A::quickSort(word *arrayData, int left, int right) {
  ...
  int i = left, j = right;
  int tmp; // для swap
  word pivot = arrayData[(left + right) / 2]; // Опорный элемент

  // Разделение массива
  while (i <= j) {
    while (arrayData[i] < pivot)
      i++;
    while (arrayData[j] > pivot)
      j--;
    if (i <= j) {
      // Обмен значениями
      tmp = arrayData[i];
...
word AS5048A::calculateMedian(word* arrayData, int sizeArray) {  
  // Сортировка массива
  quickSort(arrayData, 0, sizeArray - 1);

  // Поиск максимального разрыва для коррекции перехода через 0
  word maxGap = 0;
  int maxGapIndex = 0;
  for (int i = 1; i < sizeArray; i++) {
    word gap = arrayData[i] - arrayData[i-1];
  ...
  if (sizeArray > 1) {
      if ((sizeArray % 2) > 0) {
        median = (arrayData[(sizeArray / 2) - 1] + 
                       arrayData[sizeArray / 2] + 
                       arrayData[(sizeArray / 2) + 1]) / 3;
      } else {
        median = (arrayData[(sizeArray / 2) - 1] + 
                       arrayData[sizeArray / 2]) / 2;
      }
  } else {
    median = arrayData[0];
  }
...
word AS5048A::calculateCircular(word *adcValues, int sizeArray) {
    ...
    for (int i = 0; i < sizeArray; ++i) {
      ...
      sin_sum += sin( static_cast<double>( RotationRawToRadian(adcValues[i]) ) );
      cos_sum += cos( static_cast<double>( RotationRawToRadian(adcValues[i]) ) );
    }

    // Вычисляем круговое среднее с использованием arctan2     
    // Преобразуем среднее обратно в код АЦП
    return RadianToRotationRaw( atan2(sin_sum, cos_sum) );
}
 ...
 */
word AS5048A::read(byte sizeValues, word registerAddress, bool median) {
  word readData;
  if (sizeValues <= 0){
    sizeValues = 1;
  }
  word arrayData[sizeValues];
  ...
  if (median == true){
    ...
    for ( byte i = 0; i < sizeValues-1; i++){ 
      digitalWrite(_cs, LOW);
      arrayData[i] = SPI.transfer16(command) & ~0xC000;
      digitalWrite(_cs, HIGH);
      //Serial.println(arrayData[i], BIN);
    }

    SPI.endTransaction();
    //SPI - end transaction

    quickSort(arrayData, 0, sizeValues-1 ); //(sizeof(arrayData) / sizeof(arrayData[0]))

    if (sizeValues > 1){
      if((sizeValues % 2) > 0 ){
        readData = (arrayData[(sizeValues / 2)-1] + arrayData[sizeValues / 2]  + arrayData[(sizeValues / 2)+1]  ) / 3 ;
      }else{
        readData = ( arrayData[(sizeValues / 2)-1]  + arrayData[sizeValues / 2] ) / 2 ;
      }
    }else{
      readData = arrayData[0] ;
    }
...
word AS5048A::read(byte sizeValues, word registerAddress, bool median, bool circular) {

  if ( (sizeValues <= 0) || (!median && !circular) || (median && circular)){
    sizeValues = 1;
  }
  ...
  for ( byte i = 0; i < sizeValues;) { 
    ...
    if (_errorFlag) {
      if (error_count >= sizeValues){
        // Если количество ошибок чтения превышает размер массива выходим из цикла
        #ifdef AS5048A_DEBUG
    ...
    arrayData[i] = readData;
    i++; // Увеличиваем индекс только если чтение прошло успешно     
  }

  if (median && !circular) return calculateMedian(arrayData, sizeValues);

  if (circular && !median) return calculateCircular(arrayData, sizeValues);
